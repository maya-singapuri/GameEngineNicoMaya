...
impl Level {
    ...
    */
    ...
    /// Returns a vector of open space coordinates in the grid
    pub fn get_open_spaces(&self) -> Vec<(usize, usize)> {
        let mut open_spaces = Vec::new();
        for (y, row) in self.grid.row_iter().enumerate() {
            for (x, &tile_idx) in row.iter().enumerate() {
                let tile_data = &self.tileset[tile_idx as usize];
                if !tile_data.solid {
    ...
    // }
    pub fn grid_to_world(&self, pos: grid2::Coord) -> Vec2 {
        Vec2 {
            x: pos.0 as f32 * TILE_SZ as f32,
            y: (self.grid.height() - pos.1 - 1) as f32 * TILE_SZ as f32,
        }
    }
    pub fn world_to_grid(&self, pos: Vec2) -> grid2::Coord {
        (
